...
struct Position 
{
    ...
    bool operator==(Position rhs_) const;
    bool operator<(Position rhs_) const;

    bool IsValid() const;
    std::string ToString() const;

    static Position FromString(std::string_view str_);
...
public:
    ...
    bool operator==(FormulaError rhs_) const;
...
public:
    ...
    virtual const CellInterface* GetCell(Position pos_) const = 0;
    virtual CellInterface* GetCell(Position pos_) = 0;
    virtual Size GetPrintableSize() const = 0;

    virtual void ClearCell(Position pos_) = 0;

    virtual void PrintValues(std::ostream& output_) const = 0;
    virtual void PrintTexts(std::ostream& output_) const = 0;
};
